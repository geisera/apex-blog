...
```      | Use a colon symbol `:` to access a nested JSON node.  `%ContactInfo:FirstName%`                       |
| `"ParentObject": { "NumberMap": [ 1, 2, 3 ] }`| Use a colon symbol `:` to access nested JSON nodes and a pipe symbol ` `%ParentObject:NumberMap|3%`                    |
