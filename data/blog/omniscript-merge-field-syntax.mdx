...
1. Use existing data JSON in an element property by indicating the use of a merge field.
![Find the data you want to access.](https://www.forcedevnotes.com/blog/images/merge-field-1.png)
2. Locate the name of the JSON node in the Omniscript's data JSON.
image
3. Enter the name of the JSON node and wrap the name in percentage signs to indicate it is a merge field. For example, a merge field accessing a JSON node named firstName must use the syntax %firstName%.
image
4. Preview the Omniscript to ensure the Merge field works correctly.
image